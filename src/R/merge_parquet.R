...
classify_csv <- function(fname) {
  b <- basename(fname)
  if      (grepl("^Diversity_long_",     b)) list(analysis = "alpha", table = "diversity_long")
  else if (grepl("^Diversity_pairwise_", b)) list(analysis = "alpha", table = "diversity_pairwise")
  else if (grepl("^Diversity_",          b)) list(analysis = "alpha", table = "diversity_wide")
...
out_path <- file.path(opt$output_dir, paste0("alpha_diversity_", opt$label, ".parquet"))
